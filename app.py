    ...
    for i in range(1,2):
        ...
        for item in items:
            ...
            try:
                ...
                await newPage.waitForSelector("button[data-test-selector='download-without-license']")
                await newPage.click("button[data-test-selector='download-without-license']")
            print(f"[ENVATO DOWNLOAD {item['id']}] - Clicking download without license")
            def waitClose(page:Page):
                time.sleep(3)
                page.close()
            th = threading.Thread(target=waitClose,args=(newPage))
            th.start()
...
async def login(page):    
    ...
    await page.type("input[name='username']",os.environ.get('ENVATO_USERNAME'))
    await page.type("input[name='password']",os.environ.get('ENVATO_PASSWORD'))
